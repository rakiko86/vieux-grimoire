...
exports.signup = (req, res, next) => {
    ...
    bcrypt.hash(password, 12)
        ...
        .catch(error => res.status(500).json({ error: 'Erreur interne du serveur', details: error }));
};
    ...
    User.findOne({ email: req.body.email })
        .then(user => {
            ...
            bcrypt.compare(req.body.password, user.password)
                ...
                .catch(error => res.status(500).json({ error: 'Erreur lors de la vérification du mot de passe', details: error }));
        })
        .catch(error => res.status(500).json({ error: 'Erreur interne du serveur', details: error }));
};
